...
class Calendar
{
    ...
    function convertSolarToLunarByDate($time)
    {
        $date = date("Y-m-d", $time);
        $dates = preg_split("/-/", $date);
        return array_merge($this->convertSolarToLunar($dates[0], $dates[1], $dates[2]),$this->getJieQi($dates[0],$dates[1],$dates[2]));
    }
    ...
    */
    ...
    function getJieQiByTime($time){
        $date = date("Y-m-d",$time);
        $dates = preg_split("/-/",$date);
        return $this->getJieQi($dates[0],$dates[1],$dates[2]);
    }
}

//$lunar=new Calendar();//
//$month=$lunar->getJieQiByTime(time());//将阳历转换为阴历
//echo '<pre>';
//print_r($month);